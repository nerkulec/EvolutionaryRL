...
class SAC:
    def __init__(self, env, hidden_sizes=(256,256), activation=nn.ReLU, seed=0, 
        ...
        polyak=0.995, lr=0.001, start_steps=int(1e4), update_every=50,
        num_test_episodes=1, logger_kwargs=dict(), update_after=1000, rl_actor_copy_every=5,
        num_actors=10, elites=1, mutation_rate=0.02, mutation_prob=1, alpha=0.2,
        ...
        self.pi = SquashedGaussianMLPActor(self.obs_dim, self.act_dim, hidden_sizes, activation, self.act_limit)#.cuda()
        ...
        for p in itertools.chain(self.pi_targ.parameters(), self.q1_targ.parameters(), self.q2_targ.parameters()):
    ...
    def compute_loss_q(self, data):
        ...
        # self.stats['critic_weights'].append(list(map(to_numpy, self.q.parameters())))
    ...
    def update(self, data, timer):
        ...
        with torch.no_grad():
            for p, p_targ in zip(
                itertools.chain(self.pi.parameters(), self.q1.parameters(), self.q2.parameters()),
                itertools.chain(self.pi_targ.parameters(), self.q1_targ.parameters(), self.q2_targ.parameters())):
                p_targ.data.mul_(self.polyak)
                p_targ.data.add_((1 - self.polyak) * p.data)

    def get_action(self, o, actor=None, deterministic=False):
        with torch.no_grad():
            a = actor(FloatTensor(o), deterministic, False)[0].cpu().numpy()
    ...
    def test_agent(self, render=False, render_mode='human'):
        ...
        for j in range(self.num_test_episodes):
            ...
            while not(d or (ep_len == self.env._max_episode_steps)):
                o, r, d, _ = self.env.step(self.get_action(o, actor=self.pi, deterministic=True))
                ep_ret += r
                ep_len += 1
                if render:
    ...
    def evolution(self, rewards):
        ...
        for _ in range(len(self.actors)-self.elites): # tournament selection
            a = np.random.choice(rank[self.elites:]) # select from all
        ...
        if self.evo_num % self.rl_actor_copy_every:
            self.actors[-1] = deepcopy(self.pi)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hid', type=int, default=24)
